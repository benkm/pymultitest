...
day = f"{today.year}_{today.month}_{today.day}"


def plot_data(data, basename, model):
  E, delta_E, sigma_1_range, sigma_2_range, posterior_data, median = data
  ...
  # res_x = numpy.load(f"best_fit_graphs/{day}/{model.__name__}_best_fit_params.npy")
...
directory = f'posterior_data/{day}/'

# data_dict = {}
# GL_min = 8
# GL_max = 76.8
# points = 1000
# for model in [model14, model15]:
#   basename = f"{directory}{model.__name__}_GLmin{GL_min}_GLmax{GL_max}_points{points}"
...
# E6, E7 = data_dict["model8"][0], data_dict["model10"][0]
# deltaE_6, deltaE_7 = data_dict["model8"][1], data_dict["model10"][1]
...
for model in [model27, model28]:
  ...
  GL_max = 76.8
  points = 1000
  basename = f"{directory}{model.__name__}_GLmin{GL_min}_GLmax{GL_max}_p{points}"
