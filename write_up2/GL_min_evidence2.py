...
for i in range(len(Bbar_list)):
  ...
  plt.errorbar(GL_mins, numpy.mean(results[:, i, :], axis=1), yerr=numpy.std(results[:, i, :], axis=1), capsize=5, label=f"$\Bbar = ({Bbar_1}, {Bbar_2})$", ls='', marker='_')
...
# plt.fill_between([min(GL_mins) / 2, max(GL_mins) * 2], [2.5, 2.5], [max(results) * 1.1, max(results) * 1.1], color=color_blind_palette[2], alpha=0.2, label=r'$log(E) > 2.5$')
...
try:
  ...
  for i in range(len(Bbar_list)):
    ...
    for j, GL_min in tqdm(enumerate(GL_mins)):
      ...
      res1 = least_squares(res_function, x8, bounds=bounds8, args=(cov_inv, model1))
      ...
      res2 = least_squares(res_function, x8, bounds=bounds8, args=(cov_inv, model2))
